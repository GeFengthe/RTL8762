...
skyoldstate oldstate;       // 温湿度传感器器
static uint32_t batt_grade = 0;      //电压等级标志
//static uint8_t blink_flag = 0;      //入网界面闪烁标志
static uint8_t gap_flag = 0;        //扫描开控制标志
...
static plt_timer_t skybledisplay_timer1 = NULL;     //LCD定时器
static plt_timer_t skyblealive_timer =NULL;
static plt_timer_t skybleattr_timer = NULL;
				...
				if( attrID == tmpid ){						
					switch(attrID){
						case ATTR_CLUSTER_ID_BAT:
							...
							break;
                        case ATTR_HUMITURE_ID_TEM:
                        case ATTR_HUMITURE_ID_HUM:
                            value = MeshTxAttrStruct[i].buf[TX_ATTR_VAL_POS+3];
                            value = MeshTxAttrStruct[i].buf[TX_ATTR_VAL_POS+2];
                            value = MeshTxAttrStruct[i].buf[TX_ATTR_VAL_POS+1];
                            value = MeshTxAttrStruct[i].buf[TX_ATTR_VAL_POS];
                            break;
					}
					if( value == val ){
...
static void SkyIotReportPropertyPacket(uint16_t attrID, void *value, uint8_t seq_num)
{
	uint8_t uAttrSize = 0;
	...
	if (uAttrSize == 1){
		mIotManager.reppack_buffer[7] = *(uint8_t *)value;
	}else if (uAttrSize == 4){
		mIotManager.reppack_buffer[7] = (*(int *)value & 0xFF);
		mIotManager.reppack_buffer[8] = (*(int *)value & 0xFF00) >> 8;
		mIotManager.reppack_buffer[9] = (*(int *)value & 0xFF0000) >> 16;
		mIotManager.reppack_buffer[10] =(*(int *)value & 0xFF000000) >> 24;

//        DBG_DIRECT("---miot_rep_buff[7]=%02x,%02x,%02x,%02x, %f--",mIotManager.reppack_buffer[7],mIotManager.reppack_buffer[8],mIotManager.reppack_buffer[9],mIotManager.reppack_buffer[10],*(float *)value);
	}
...
//            gap_sched_scan(false); //开机后关闭扫描
            blemesh_unprov_ctrl_dlps(true); 
            break;
        }
        case DISPLAY_LCD_TIMEOUT:                    //显示控制定时器
            SkyBleMesh_DisPlayTempHum();
            break;
...
//		if(	mIotSaveParams.tem != mIotManager.mLightManager.tem
//			|| mIotSaveParams.bat != mIotManager.mLightManager.bat
//            || mIotSaveParams.hum != mIotManager.mLightManager.hum){
...
//			mIotSaveParams.hum = mIotManager.mLightManager.hum;
//			mIotSaveParams.bat = mIotManager.mLightManager.bat;
//            mIotSaveParams.tem = mIotManager.mLightManager.tem;
//		}
//		
//        oldtick = newtick;
//    }
...
{	
	...
	memcpy(buffer + offset, &(mIotManager.mLightManager.tem), 4);
	offset += 4;
	utils_md5_update(context, (unsigned char*)&(mIotManager.mLightManager.tem), 4);
    
    memcpy(buffer + offset, &(mIotManager.mLightManager.hum), 4);
	offset += 4;
	utils_md5_update(context, (unsigned char*)&(mIotManager.mLightManager.hum), 4);
...
static int SkyBleMesh_ReadConfig(void)
{
	...
	//
	memcpy(&(mIotManager.mLightManager.tem), buffer + offset, 4);	
	offset += 4;
	utils_md5_update(context, (unsigned char*)&(mIotManager.mLightManager.tem), 4);
	//
    memcpy(&(mIotManager.mLightManager.hum), buffer + offset, 4);	
	offset += 4;
	utils_md5_update(context, (unsigned char*)&(mIotManager.mLightManager.hum), 4);
...
{
    mIotManager.mLightManager.bat = 0.0;
    mIotManager.mLightManager.tem = 0.0;
    mIotManager.mLightManager.hum = 0;
}
...
static void Reset_iotmanager_para(void)
{
	...
	// 上报设置项，做同步
	mIotManager.report_flag |= BLEMESH_REPORT_FLAG_TEM;
    mIotManager.report_flag |= BLEMESH_REPORT_FLAG_HUM;
	mIotManager.report_flag |= BLEMESH_REPORT_FLAG_BAT;
	mIotManager.tem_seq = 0;
    mIotManager.hum_seq = 0;
...
static void skyble_alive(void)
{
    if(gap_flag == 0)
    {
        alive_ctrl_dlps(false);
        SkyBleMesh_alivetimer();
        gap_sched_scan(false);
        gap_sched_scan(true);
        os_delay(2);
        gap_flag = 1;
...
{	
	...
	if (alivetimecnt >= 150){  // 30*50ms
		...
		if (mIotManager.alive_status == 1){
			int sub_timeout_ms;
			sub_timeout_ms = HAL_CalculateTickDiff(mIotManager.recv_alive_tick, tick);
			if (sub_timeout_ms >= DEFAULT_GATEWAY_ALIVE_MS){        //10min 掉线
...
static void Main_Upload_State(void)
{
	if (mIotManager.alive_status == 1){	
		...
		if(mIotManager.report_flag & BLEMESH_REPORT_FLAG_TEM){							// 开关状态发生改变
			mIotManager.report_flag &= ~ BLEMESH_REPORT_FLAG_TEM;
			SkyIotReportPropertyPacket(ATTR_HUMITURE_ID_TEM, &mIotManager.mLightManager.tem, mIotManager.tem_seq);	
			mIotManager.tem_seq = 0;
            mIotSaveParams.temtime = HAL_GetTickCount();
		}
        if(mIotManager.report_flag & BLEMESH_REPORT_FLAG_HUM){
            mIotManager.report_flag &= ~ BLEMESH_REPORT_FLAG_HUM;
            SkyIotReportPropertyPacket(ATTR_HUMITURE_ID_HUM, &mIotManager.mLightManager.hum, mIotManager.hum_seq);
            mIotManager.hum_seq = 0;
            mIotSaveParams.humtime = HAL_GetTickCount();
        }
...
//// 温湿度转换函数
//void SkyBleMesh_ReadTempHum(void)
//{
//    int temptmp;
//    uint32_t humtmp;
//    SHTC3_Read_Temp_Hum(&temptmp,&humtmp);
//    mIotManager.mLightManager.tem = (float)temptmp/10.0;
//    mIotManager.mLightManager.hum = (float) humtmp/10.0;
//}
...
//	}
	#endif
    SkyBleMesh_BattWaringTx(1000*60*22);              //电池电量发送 22min
...
static void SkyBleMesh_DisPlay_timer1_cb(void *time)
{			
    T_IO_MSG msg;
    msg.type = IO_MSG_TYPE_TIMER;
    msg.subtype = DISPLAY_LCD_TIMEOUT;
    app_send_msg_to_apptask(&msg);
}

void SkyBleMesh_DisPlay_timer1(uint32_t lcd_time)
{	
	if(skybledisplay_timer1 == NULL){
		skybledisplay_timer1 = plt_timer_create("main", lcd_time, true, 0, SkyBleMesh_DisPlay_timer1_cb);
		if (skybledisplay_timer1 != NULL){
			plt_timer_start(skybledisplay_timer1, 0);
		}
	}
}
...
extern void SkyBleMesh_Batterval_Lightsense(bool onlybatt)
{
	uint16_t batt_val=0;
        ...
        if(batt_val>=2900)
        {
            batt_grade = BATT_TERY_5;
        }else if((batt_val >=2800)&&(batt_val <2900))
        {
            batt_grade = BATT_TERY_4;
        }else if((batt_val >= 2600)&&(batt_val < 2800))
        {
            batt_grade = BATT_TERY_3;
        }else if((batt_val >= 2400)&&(batt_val <2600))
        {
            batt_grade = BATT_TERY_2;
        }else if((batt_val >= 2200)&&(batt_val <2400))
        {
            batt_grade = BATT_TERY_1;
        }else if(batt_val <2200)
        {
            batt_grade = BATT_TERY_0;
        }
//        DBG_DIRECT("----batt_grade=%d----\r\n",batt_grade);
		if(mIotManager.batt_rank == BATT_NORMAL){	
			if(batt_val <= BATT_WARING_RANK+200){       //2.2V
                mIotManager.mLightManager.bat =1;           //低电量标志置位
            ...
            if(batt_val > BATT_WARING_RANK + 200)
            {
...
extern void SkyBleMesh_BattWaringTx(uint32_t ms)
{
    ...
    if(oldtick ==0)
    {
        oldtick = HAL_GetTickCount();   //得到初始时间
    }
    tick = HAL_GetTickCount();
    tickcnt = HAL_CalculateTickDiff(oldtick,tick);
    if(tickcnt >= ms)
    {
       mIotManager.report_flag |= BLEMESH_REPORT_FLAG_BAT;
       mIotManager.bat_seq = 0;
...
//上电启动是否配网函数
static int temp = 0;        //屏幕温度值
static uint32_t hum = 0;        //屏幕湿度值
static int temp_old =0;         //上次记录数据
static uint32_t hum_old =0 ;    //
static uint8_t process_flag =0;         //入网图标闪标志
static uint8_t lcd_open = 0;            //上电第一次打开屏幕标记
static uint32_t old_time = 0;         //开机时间

//提取温湿度传感器值
void SkyBle_ReadSHTC3(void)
{
    SHTC3_Read_Temp_Hum(&temp,&hum);
    mIotManager.mLightManager.tem = ((float)temp/(float)10.0);
    mIotManager.mLightManager.hum = ((float)hum/(float)10.0);
}
//开机亮屏函数
void SkyLcd_Process(void)
{
    SkyIot_Lcd_Display(hum,temp,0,batt_grade);
    lcd_open = 1;
    if(mIotManager.process_state == 0x55)
    {
        process_flag = 1;
        SkyBleMesh_DisPlay_timer1(1000);        
    }else{
        SkyBleMesh_DisPlay_timer1(2000);               //正常开机10s后刷新一次屏
    }
...
//    int lcdtmp =0;
    ...
    if(temp >= temp_old)
    {
        tempcnt = temp -temp_old;
    }else{
        tempcnt = temp_old - temp;
    }
    if(hum >= hum_old)
    {
        humcnt = hum - hum_old;
...
//    SkyBleMesh_lcdexit();
    if(mIotManager.process_state == 0x55)
    {
        if((tempcnt >3)||(humcnt >6))
        {
            lcdhum = (hum +hum_old)/2;
            temp_old = temp;
            hum_old = lcdhum;
        }
        lcdhum = hum_old;
        if(process_flag == 1)
        {
            SkyIot_Lcd_Display(lcdhum,temp_old,120,batt_grade);
            process_flag =0;
        }else{
            SkyIot_Lcd_Display(lcdhum,temp_old,0,batt_grade);
            process_flag =1;
        }
    }else{
        SkyBleMesh_TempHumTx(1000*60*6,tempcnt,humcnt);
        if(mIotManager.alive_status == 0x01)
        {
            single =120;
        }else
        {
        ...
        if((tempcnt >3) || (humcnt>5))
        {
//            lcdtmp = (temp_old + temp)/2;
            lcdhum = (hum_old + hum)/2;
            DBG_DIRECT("--------single=%d-------\r\n",single);
            SkyIot_Lcd_Display(lcdhum,temp,single,batt_grade);
//            tempcnt = 0;
            hum_old =lcdhum;
            temp_old =temp;
        }
        if(lcd_open ==1)
        {
//            plt_timer_delete(skybledisplay_timer1,0);
//            SkyBleMesh_DisPlay_timer1(1000*1);
            lcd_open = 0;
        }
    }
    DBG_DIRECT("----LCD_UPDATE----\r\n");
}

void SkyBleMesh_ReadOpentime(void)
{
    old_time = HAL_GetTickCount();
}
//获取时间差函数
bool SkyBleMesh_Ostimecnt(uint32_t time_ms)
{
    uint32_t tick = 0;
    uint32_t subtime = 0;
    uint8_t ret =false;
    ...
    tick = HAL_GetTickCount();
    subtime = HAL_CalculateTickDiff(old_time,tick);
    if(subtime > time_ms)
    {
        ret = true;
        old_time = HAL_GetTickCount();
...
//上报温湿度属性控制函数 根据时间控制
void SkyBleMesh_TempHumTx(uint32_t time_ms,int temcnt,uint32_t humcnt)
{
    float tcnt =0;
    float hcnt = 0;
    uint32_t tick =0;
    uint32_t tickcnt =0;
    if(mIotManager.mLightManager.tem >= mIotSaveParams.tem)
    {
        tcnt = mIotManager.mLightManager.tem - mIotSaveParams.tem;
    }else{
        tcnt = mIotSaveParams.tem - mIotManager.mLightManager.tem;
    }
    if(mIotManager.mLightManager.hum >= mIotSaveParams.hum)
    {
        hcnt = mIotManager.mLightManager.hum - mIotSaveParams.hum;
    }else{
        hcnt = mIotSaveParams.hum - mIotManager.mLightManager.hum;
    }
    tick =HAL_GetTickCount();
    if( (float)(tcnt-(float)0.5)> (float)0.000001)
    {
        DBG_DIRECT("--------temcnt=%f----\r\n",tcnt);
        tickcnt = HAL_CalculateTickDiff(mIotSaveParams.temtime,tick);
        if(tickcnt >5000)
        {
            mIotManager.report_flag |= BLEMESH_REPORT_FLAG_TEM;
            mIotManager.tem_seq = 0;
    ...
    if((float)(hcnt -(float)0.7)>(float)0.00001)
    {
        DBG_DIRECT("--------humcnt=%f----\r\n",hcnt);
        tickcnt = HAL_CalculateTickDiff(mIotSaveParams.humtime,tick);
        if(tickcnt >5000)
        {
            mIotManager.report_flag |= BLEMESH_REPORT_FLAG_HUM;
            mIotManager.hum_seq = 0;
...
//    if(SkyBleMesh_Ostimecnt(1000*60*6))                 //4min上报一次当前属性
//    {
//        mIotManager.report_flag |= BLEMESH_REPORT_FLAG_TEM;
//        mIotManager.report_flag |= BLEMESH_REPORT_FLAG_HUM;
//        mIotManager.tem_seq = 0;
//        mIotManager.hum_seq = 0;
//    }else{
//        if(mIotManager.mLightManager. >3)
//        {
//            mIotManager.report_flag |= BLEMESH_REPORT_FLAG_TEM;
//            mIotManager.tem_seq = 0;
//        }
...
static void SkyBleMesh_alivetime_cb(void *timer)
{
    DBG_DIRECT("--------skyblealive_timer is ok---\r\n");
    alive_ctrl_dlps(true);
    gap_flag =0;
    if(skyblealive_timer != NULL)
    {
        plt_timer_delete(skyblealive_timer,0);
    }
}

//心跳超时控制定时器
void SkyBleMesh_alivetimer(void)
{
    if(skyblealive_timer ==NULL)
    {
        skyblealive_timer = plt_timer_create("alive",3000,false,0,SkyBleMesh_alivetime_cb);
    }
    plt_timer_start(skyblealive_timer,0);
}
...
static void SkyBleMesh_attrtime_cb(void *timer)
{
    blemesh_report_ctrl_dlps(true);
    if(skybleattr_timer != NULL)
    {
        plt_timer_delete(skybleattr_timer,0);
    }
}

void SkyBleMesh_attrdelettimer(void)
{
        if(skybleattr_timer != NULL)
    {
        plt_timer_delete(skybleattr_timer,0);
...
//属性接收超时控制定时器
void SkyBleMesh_attrtimer(void)
{
    if(skybleattr_timer == NULL)
    {
        skybleattr_timer = plt_timer_create("attr",3000,false,0,SkyBleMesh_attrtime_cb);
    }
    plt_timer_start(skybleattr_timer,0);
}

//void SkyBleMesh_aliveCtrl(uint32_t alivecnt)
//{
...
//        if(alive_time == 0)
//        {
//            alive_time = HAL_GetTickCount();
//        }
//        tick = HAL_GetTickCount();
//        subtime = HAL_CalculateTickDiff(alive_time,tick);
//        if(subtime > alivecnt)
//        {
//            alive_ctrl_dlps(true);
//        }
