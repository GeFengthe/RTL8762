...
#define GDMA_MULTIBLOCK_MODE            AUTO_RELOAD_TRANSFER
//#define GDMA_MULTIBLOCK_MODE            LLI_WITH_AUTO_RELOAD_SAR
//#define GDMA_MULTIBLOCK_MODE            LLI_WITH_AUTO_RELOAD_DAR
...
void driver_gdma_multiblock_init(void)
{
    ...
    GDMA_InitStruct.GDMA_SourceAddr         = (uint32_t)GDMA_Send_Buffer[2];
    GDMA_InitStruct.GDMA_DestinationAddr    = (uint32_t)GDMA_Recv_Buffer[4];
...
#if GDMA_MULTIBLOCK_MODE != AUTO_RELOAD_TRANSFER
    for (uint32_t i = 0; i < 12; i++)
    {
        if (i == 11)
        {
            ...
            GDMA_LLIStruct[i].LLP = (uint32_t)GDMA_LLIStruct;
