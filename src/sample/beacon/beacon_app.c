...
/** @addtogroup  PERIPH_SEVER_CALLBACK Profile Server Callback Event Handler
    ...
    * @{
    */
T_SERVER_ID beacon_srv_id; /**< Simple ble service id*/
...
#if defined(DEVICE_CONNECTABLE)
bool modify_adv_interval = false;
#endif
...
void app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_UART:
        length = (uint8_t)io_msg.subtype;
        data_handle((uint8_t *)rx_char, length);
#endif
    default:
        break;
    }
...
#if defined(DEVICE_CONNECTABLE)
                if (modify_adv_interval)
                {
                    modify_adv_interval = false;
                    le_adv_set_param(GAP_PARAM_ADV_INTERVAL_MIN, sizeof(adv_interval), &adv_interval);
                    le_adv_set_param(GAP_PARAM_ADV_INTERVAL_MAX, sizeof(adv_interval), &adv_interval);
                    le_adv_start();
                }
#endif
            }
...
T_APP_RESULT app_profile_callback(T_SERVER_ID service_id, void *p_data)
{
    ...
    else if (service_id == beacon_srv_id)
    {
        T_BEACON_CALLBACK_DATA *p_beacon_cb_data = (T_BEACON_CALLBACK_DATA *)p_data;
        switch (p_beacon_cb_data->msg_type)
        {
        case SERVICE_CALLBACK_TYPE_READ_CHAR_VALUE:
            if (p_beacon_cb_data->msg_data.read_value_index == BEACON_READ_V1)
            {
                uint8_t value[4] = {adv_data[25], adv_data[26], adv_data[27], adv_data[28]};
                APP_PRINT_INFO0("READ");
                beacon_sample_service_set_parameter(BEACON_SAMPLE_SERVICE_PARAM_V1_READ_CHAR_VAL, 4, &value);
            }
            break;
        case SERVICE_CALLBACK_TYPE_WRITE_CHAR_VALUE:
            switch (p_beacon_cb_data->msg_data.write.opcode)
            {
            case BEACON_WRITE_V2:
                break;
            default:
                break;
            }
...
/** @} */ /* End of group BEACON_SEVER_CALLBACK */
/** @} */ /* End of group BEACON_APP */
