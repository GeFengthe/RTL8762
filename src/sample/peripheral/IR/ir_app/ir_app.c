...
T_SERVER_ID ads_srv_id; /**< Audio down streaming service id*/
    ...
    case IO_MSG_TYPE_IR:
        {
            ...
            {
                if (ir_learn_msg_proc(io_msg.subtype))
                {
                    // IR learn finish
...
#ifdef AUDIO_OVER_BLE_SUPPORT
            audio_update_conn_params(AUDIO_CONNECT_INTERVAL, AUDIO_CONNECT_LATENCY,
                                     AUDIO_SUPERVISION_TIMEOUT);
#endif
        }
        break;
...
void app_handle_conn_mtu_info_evt(uint8_t conn_id, uint16_t mtu_size)
{
    ...
    {
        /* update mtu size */
        audio_global_data.mtu_size = mtu_size;
    }
    APP_PRINT_INFO1("[app_handle_conn_mtu_info_evt] update mtu size to %d", mtu_size);
#endif
}
...
#ifdef AUDIO_OVER_BLE_SUPPORT
    else if (service_id == ads_srv_id)
    {
        extern T_APP_RESULT app_ads_srv_cb(T_ADS_CALLBACK_DATA * p_data);
        app_result = app_ads_srv_cb((T_ADS_CALLBACK_DATA *)p_data);
    }
#endif
    return app_result;
}
