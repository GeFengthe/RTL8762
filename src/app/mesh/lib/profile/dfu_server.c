...
void dfu_server_adv_send(void)
{
    ...
    padv_data->dfu.patch_version = DFU_VERSION((T_IMG_ID)RomPatch);
        ...
        dfu_ctx.curr_offset + ota_tmp_buf_used_size == dfu_ctx.image_length)
    {
        ...
        else //crc ok
        {
            ...
            else
            {
                ...
                do
                {
                    sil_dfu_flash_erase(dfu_ctx.signature, gSilDfuResendOffset);
                    //delay_ms_asm(200);
                    signal = os_lock();
                ...
                if ((dfu_ctx.curr_offset - gSilDfuResendOffset) > SECTOR_SIZE) //need erase two sector
                {
                    ...
                    do
                    {
                        ...
                        sil_dfu_flash_erase(dfu_ctx.signature, gSilDfuResendOffset + SECTOR_SIZE);
                        signal = os_lock();
...
void  dfu_server_handle_cp(uint16_t length, uint8_t *p_value)
{
    ...
    case DFU_OPCODE_START_DFU:
        ...
        {
#if DFU_WO_SCAN
            gap_sched_params_get(GAP_SCHED_PARAMS_BG_SCAN, &dfu_ctx.bg_scan, 1);
            if (dfu_ctx.bg_scan)
            {
                gap_sched_scan(0);
            }
    ...
    case DFU_OPCODE_VALID_FW:
        if (length == DFU_LENGTH_VALID_FW)
        {
            ...
            check_result = silent_dfu_check_checksum(dfu_ctx.signature);
    ...
    case DFU_OPCODE_REPORT_TARGET_INFO:
        if (length == DFU_LENGTH_REPORT_TARGET_INFO)
        {
            ...
            dfu_ctx.origin_image_version = dfu_ctx.signature == AppPatch ? DFU_APP_VERSION : DFU_VERSION((
                                               T_IMG_ID)dfu_ctx.signature);
