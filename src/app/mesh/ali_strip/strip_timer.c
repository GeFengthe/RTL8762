...
periodic_data_t period_list[40];
...
void vTimerReportCallback(void *pxTimer)
{
    ...
    bee_io_msg.subtype = strip_data_current.strip_on_off;
...
void unix2UTC(uint32_t unix_time)
{
    ...
    RTC_ResetCounter();
    RTC_init();
    RTC_SetComp(RTC_COMP_INDEX, RTC_GetCounter() + (60 - unix_time % 60) * 10);
    ...
    local_time.minutes = sec_diff / MINUTE;
    local_time.seconds = sec_diff = sec_diff % MINUTE;
...
void timer_list_init(void)
{
    for (int i = 0; i < TIMER_MAXIMUM; i++)
    {
        ...
        timer_list[i].attr_type = 0xFFFF;
    }
//    flash_read_locked(FLASH_SAVE_ADDR, sizeof(timer_data_t) * 40, (uint8_t *) timer_list);
//    flash_read_locked(FLASH_SAVE_ADDR+PERIOD_OFFSET, sizeof(periodic_data_t) * 40, (uint8_t *) period_list);
}
...
void period_list_init(void)
{
    for (int i = 0; i < TIMER_MAXIMUM; i++)
    {
        period_list[i].index = 0xFF;
        period_list[i].relative_time = 0xFFFF;
        period_list[i].on_off = GENERIC_OFF;
        period_list[i].attr_type = 0xFFFF;
        period_list[i].schedule = 0;
    }
//    flash_read_locked(FLASH_SAVE_ADDR, sizeof(timer_data_t) * 40, (uint8_t *) timer_list);
//    flash_read_locked(FLASH_SAVE_ADDR+PERIOD_OFFSET, sizeof(periodic_data_t) * 40, (uint8_t *) period_list);
}
 ...
 * @note each time this function is invoked, timer list will be rewrited into falsh
*/
void period_sort(void)
{
    uint8_t i, j, min;
    periodic_data_t temp_timer;
    for (i = 0; i < TIMER_MAXIMUM; i++)
    {
        min = i;
        for (j = i + 1; j < TIMER_MAXIMUM; j++)
        {
            if (period_list[min].relative_time > period_list[j].relative_time)
            {
                min = j;
            }
        }
        if (min != i)
        {
            temp_timer = period_list[i];
            period_list[i] = period_list[min];
            period_list[min] = temp_timer;
        }
    }
//    if (!flash_erase_locked(FLASH_ERASE_SECTOR, 0x00830000))
//    {
...
{
    if (time_data.unix_time < strip_data_current.unix_time)
    {
        return NOT_REACHABLE_TIME;
    }
    if (strip_data_current.unix_time == 0xFFFFFFFF)
    {
    ...
    for (int i = 0; i < 40; i++)
    {
        if (timer_list[i].unix_time == time_data.unix_time)
        {
...
ERROR_TYPE is_in_period_list(periodic_data_t time_data)
{
    for (int i = 0; i < 40; i++)
    {
        if (period_list[i].relative_time == time_data.relative_time)
        {
            period_list[i] = time_data;
            return ADD_SUCCESS;
        }
        if (period_list[i].relative_time == 0xFFFF && timer_list[i].index == 0xFF)
        {
            period_list[i] = time_data;
            period_sort();
            return ADD_SUCCESS;
...
void check_timer_valid(void)
{
    ...
    for (int i = 0; i < TIMER_MAXIMUM; i++)
    {
        if (timer_list[i].unix_time <= strip_data_current.unix_time)
        {
            timer_list[i].index = 0xFF;
            timer_list[i].unix_time = 0xFFFFFFFF;
            timer_list[i].on_off = GENERIC_OFF;
