...
void app_handle_io_msg(T_IO_MSG io_msg)
{
    ...
    case IO_MSG_TYPE_RTC:
        ...
        case IO_MSG_TIMER_RWS:
            ...
            strip_data_current.unix_time += 60;
            ...
            //Update system time
            if (strip_data_current.has_updated == false)
            {
                if (strip_data_current.need_retry)
                {
                    retry_period++;
                    if (retry_period == strip_data_current.time_sync_retry_delay)
                    {
                        if (retry_times <= strip_data_current.time_sync_retry_times)
                        {
                            send_update_request();
                            retry_times++;
                        }
                ...
                {
                    if ((strip_data_current.unix_time - sys_clk_update) == MINUTE * strip_data_current.time_sync_period)
                    {
                        send_update_request();
                        strip_data_current.need_retry = true;
                    }
                }
            }
            else
            {
                retry_period = 0;
                retry_times = 0;
                strip_data_current.has_updated = false;
                strip_data_current.need_retry = false;
            }
            ...
            {
                if (period_list[i].schedule == 0)
                {
                    break;
                }
                if (weekday_list[local_time.weekday] & period_list[i].schedule)
                {
                    if ((local_time.minutes + local_time.hour * 60) == period_list[i].relative_time)
                    {
                        strip_data_current.strip_on_off = period_list[i].on_off ? GENERIC_ON : GENERIC_OFF;
                        T_IO_MSG on_off_msg;
                        on_off_msg.type = IO_MSG_TYPE_GPIO;
                        on_off_msg.subtype = strip_data_current.strip_on_off;
            ...
            if (strip_data_current.unix_time >= timer_list[0].unix_time)
            {
                strip_data_current.strip_on_off = timer_list[0].on_off ? GENERIC_ON : GENERIC_OFF;
                T_IO_MSG on_off_msg;
                on_off_msg.type = IO_MSG_TYPE_GPIO;
...
#if ALI_AIS_SUPPORT
            ais_server_disconnect_cb(conn_id);
#endif
